...
pythagorean = [(m*m-n*n,2*m*n,m*m+n*n)|
  m <- [1..],n <- [1..(m-1)], -- dit bepaalt de volgorde!
  ggd m n == 1,
  even m || even n,
  odd m || odd n]
...
-- de lijst kan ook oneindig zijn
prefixes::[a] -> [[a]]
prefixes = myFoldr (\x y -> [x]:map (x:) y) [] -- kan je indices ook zo schrijven?
...
freq s = go (cnt s (replicate 26 0.0))
  where
    ...
    cnt (x:s) fs =
      if isLower x
        then cnt s (increase (let2int x) fs)
        else cnt s fs
...
mnlc3 f n = concat (map (\x -> 
                concat (map (\y -> 
                   (map (f x y) (filter (\c -> x*x + y*y == c*c) [y..n])) ) [x..n])) 
                      (filter even [1..n])) 
