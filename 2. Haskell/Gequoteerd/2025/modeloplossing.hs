...
testCatalan s e =
  let ls l = (take (e + 1 - s) (drop s l))
   in [(c == r, abs ((fromInteger c) - a) / (fromInteger c)) | (c, r, a) <- zip3 (ls lcatalan) (ls lcatalanRec) (ls lcatalanAsy)]
...
cantor p =
  let prev = (cantor (p - 1))
   in concat [prev, take (3 ^ (p - 1)) (repeat '.'), prev]
...
logBaseInt b x = go b x 1 0
  where
    go g o v acc =
      if (v * g) <= o
        then go g o (v * g) (acc + 1)
        else acc
...
zoomieCan = do
  ...
  let myCantor = (zoomCantor g)
   in showTheZoomie g myCantor
