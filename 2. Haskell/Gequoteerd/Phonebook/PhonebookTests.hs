...
-- Required for older versions of GHC
...
import Data.Monoid
import Phonebook
import Prelude hiding (catch, foldl1, foldr1)
...
main =
  startTests
    ...
    <> test
      "(owner (emptyBook bill) == Just bill)"
    ...
    <> test
      ...
      (byName "Bob" (addToBook bob (emptyBook bill)) == Just bob)
    <> test
      "(byPhone [32,444,123] (addToBook bob (emptyBook bill) == Just bob)"
    ...
    <> test
      ...
      (byPhone [32, 444, 126] valbook == Nothing)
    <> test
      "(number (callerID billbook) == 32 444 123)"
