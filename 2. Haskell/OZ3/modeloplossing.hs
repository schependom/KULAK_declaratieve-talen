...
prog1 = do m <- readLn
           n <- readLn
           zet m n
        where zet::Int -> Int -> IO ()
              zet m n = if m == 0 then return ()
                          else do print n
                                  zet (m-1) n
...
doMenu p (x:xs) = do putStrLn (show p ++ ". " ++ fst x)
                     doMenu (p+1) xs
...
maakLijnen Nil _ _ = []
maakLijnen (Node x l r) iacc pref  = [iacc ++ pref ++ show x] 
           ++ maakLijnen l (iacc ++ blanc pref) "|-- " 
           ++ maakLijnen r (iacc ++  blanc pref) "'-- "
           where blanc::String -> String
                 blanc ""     = ""
                 blanc "|-- " = "|   "
                 blanc "'-- " = "    "
...
maakBoom 0 _ = Nil
maakBoom n g = let (v,gt) = randomR (0,n-1) g
                   (gl,gr) = split gt
...
behandel mnBoom = do nwBoom <- menu2
                         [("Voegtoe", \b -> do
                                            putStr "> "
                                            v <- readLn
