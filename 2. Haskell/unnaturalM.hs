...
data UnnaM a = U [a] deriving (Eq, Ord, Show)
...
-- Test de wetten
testLaws = [(theDouble 7581) == (return 7581 >>= theDouble), 
            uI == (uI >>= return),
...
testLaw3 t1 t2 u = do putStrLn ((show (u >>= t1 >>= t2)) ++ "==" ++ (show (u >>= (\x -> ((t1 x) >>= t2)))))
...
testnnLaws = [(theDouble 7581) == (return 7581 !>>= theDouble), 
            uI == (uI !>>= return),
            ((U [1,2,3]) !>>= theDouble !>>= theTriple) == ((U [1,2,3]) !>>= (\x -> ((theDouble x) !>>= theTriple))),
            ((U [1,2,3]) !>>= (f 15) !>>= g) == ((U [1,2,3]) !>>= (\x -> ((f 15 x) !>>= g)))]
...
testnnLaw3 t1 t2 u
 = do putStrLn ((show (u !>>= t1 !>>= t2)) ++ "==" ++ (show (u !>>= (\x -> ((t1 x) !>>= t2)))))
      print ((u !>>= t1 !>>= t2) == (u !>>= (\x -> ((t1 x) !>>= t2))))
...
doTests = do putStrLn "=============== testen op de Monad UnnaM ================"
...
bbJoin (B c l r) = if (l < r) then B (c+1) (bbJoin l) (bbJoin r)
...
testNatlB = [testNaturalB (gb 5) someBB, 
             testNaturalB (fbi 15 5) someBB,
             testNaturalB (fbs 15 5) someBB]

-- Test de wetten
testLawsB = [(gb 5 81) == (return 81 >>= (gb 5)),
            someB == (someB >>= return),
...
testBLaw3 t1 t2 u = do putStrLn ((show (u >>= t1 >>= t2)) ++ "==" ++ (show (u >>= (\x -> ((t1 x) >>= t2)))))
                       print ((u >>= t1 >>= t2) == (u >>= (\x -> ((t1 x) >>= t2))))
-- In de versie met Ord a
testbbLawsB = [(gb 5 81) == (return 81 !!>>= (gb 5)),
               someB == (someB !!>>= return),
               (someB !!>>= (gb 5) !!>>= (fbi 3 2)) == (someB !!>>= (\x -> ((gb 5 x) !!>>= (fbi 3 2)))),
               (someB !!>>= (fbi 3 2) !!>>= (gb 5)) == (someB !!>>= (\x -> ((fbi 3 2 x) !!>>= (gb 5))))]
-- Een test op de naturaliteit
...
testbbNatlB = [testbbNatural (gb 5) someBB, 
               testbbNatural (fbi 15 5) someBB, 
               testbbNatural (fbs 15 5) someBB]

doTestsB = do putStrLn "=============== testen op de Monad BnnaM ================"
              putStrLn "Test op de MonadPlus wetten ([eerste,tweede,derde,derde])"
              print testLawsB
              putStrLn "Test op naturaliteit"
              print testNatlB
